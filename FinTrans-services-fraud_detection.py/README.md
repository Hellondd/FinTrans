...
2. **Антифрод-сервис**
   - 6 факторов анализа
   - Fraud_score ≥ 0.6 → блокировка
   - Fraud_score 0.4–0.6 → ручное ревью

3. **Скоринг-сервис (кредитный рейтинг)**
   - Формула: доход(0.25) + возраст(0.1) + стабильность(0.25) + транзакции(0.3) + продукты(0.1)

4. **Дашборды и аналитика**
